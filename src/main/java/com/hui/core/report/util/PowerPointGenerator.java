...
public class PowerPointGenerator {
     ...
     */
    public static void generatorPowerPoint(String templateFilePath, String destFilePath, Map<Integer, SlideData> slideDataMap) throws IOException {
...
//        XSLFSlide slide1 = ppt.createSlide();
        List<POIXMLDocumentPart> partList = new ArrayList<POIXMLDocumentPart>();
    ...
    private static void generatorSlide(XSLFSlide slide, SlideData slideData) {
        ...
        for (XSLFShape shape : shapeList) {
            ...
            if (shape instanceof XSLFTable) {
                ...
                XSLFTableRow row =  ((XSLFTable) shape).getRows().get(0);
...
//        rows.get(0) 是标题
    ...
    private static void generatorLineGraph(XSLFChart chart, List<ChartSeries> seriesDataList) {
        ...
        CTLineChart lineChart1 = plotArea.getLineChartList().get(1);

        // 允许自定义颜色
        CTBoolean ctBoolean = lineChart.addNewVaryColors();
        ctBoolean.setVal(true);

        CTBoolean ctBoolean1 = lineChart1.addNewVaryColors();
        ctBoolean1.setVal(true);
        ...
        if (seriesDataList.size() > lineChart.getSerList().size()) {
            ...
            for (int i = 0; i < addTimes; i++) {
                ...
                CTShapeProperties properties = ctLineSer.addNewSpPr();
        ...
        if (seriesDataList.size() > lineChart1.getSerList().size()) {
            int addTimes = seriesDataList.size() - lineChart1.getSerList().size();
            for (int i = 0; i < addTimes; i++) {
                CTLineSer ctLineSer = lineChart1.addNewSer();
        ...
        if (lineChart1.getSerList().size() > seriesDataList.size()) {
            int removeTimes = lineChart1.getSerList().size() - seriesDataList.size();
            for (int i = 0; i < removeTimes; i++) {
                lineChart1.removeSer(lineChart1.getSerList().size() - 1);
            }
        }

        List<CTLineSer> serList = lineChart.getSerList();
        List<CTLineSer> serList1 = lineChart1.getSerList();
        if (serList == null || serList1 == null  ) {
        ...
        for (int i = 0; i < seriesDataList.size(); i++) {
            ...
            hex.setStringValue("FFD0" + (i+30));
            lineSer.addNewSpPr().addNewLn().addNewSolidFill().addNewSrgbClr().xsetVal(hex);
        ...
        for (int i = 0; i < seriesDataList.size(); i++) {
            ...
            CTLineSer lineSer = serList1.get(i);

            STHexBinary3 hex = STHexBinary3.Factory.newInstance();
            hex.setStringValue("62D1" + (i+30));
            lineSer.addNewSpPr().addNewLn().addNewSolidFill().addNewSrgbClr().xsetVal(hex);
